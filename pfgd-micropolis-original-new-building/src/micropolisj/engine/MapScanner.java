...
class MapScanner extends TileBehavior
{
	...
	public static enum B
	{
		...
		AIRPORT,
		SEAPORT,
		HALFWAY; //Placeholder enum for HALFWAY. Change to building name if making a HALFWAY
	}
	...
	public void apply()
	{
		...
		case SEAPORT:
			...
			return;
		case HALFWAY:
			doHalfway(); //Call the HALFWAY placeholder function
			return;
		default:
			assert false;
		}
	...
	void doHalfway()
	{
		...
		boolean powerOn = checkZonePower();
		if ((city.cityTime % 8) == 0) {
			repairZone(HALFWAY, 3);
		}
		
		city.halfwayHouseCount += 1;
		city.policeEffect -= 20;
		//counts as a residential zone
		city.resZoneCount++;
		
		// finding out population of halfway house
		int tpop;
		if(tile == HALFWAY) {
		...
		// updating census statistics
		city.resPop += tpop;
		
		// handling traffic, the ex-cons don't care about bad traffic as much
		int trafficGood;
		...
		{		
			...
			if (zscore > -350 && zscore - 26380 > (PRNG.nextInt(0x10000)-0x8000))
			{
				...
				// make new normal residential zone and get rid of halfway house
				residentialPlop(tpop, value);
				city.halfwayHouseCount -= 1;
				city.policeEffect += 20;
				return;
			}
