...
public class Micropolis
{
	...
	int seaportCount;
	int airportCount;
	int halfwayHouseCount;
	
	// used to see if halfway house should convert into normal residential
	public boolean cityFullyFunded;
	...
	void initTileBehaviors()
	{
		...
		bb.put("HALFWAY", new MapScanner(this, MapScanner.B.HALFWAY));
	...
	public BudgetNumbers generateBudget()
	{
		...
		// road request, function of maintenance costs, # of roads, and # of halfway houses
		if(lastRoadTotal > 0) {
			b.roadRequest = (int)Math.round((lastRoadTotal + lastRailTotal * 2) * RLevels[gameLevel]) + (halfwayHouseCount * 3);
		}
		else {
			b.roadRequest = 0;
		}
		
		// fire request, function of maintenance costs, # of stations, and # of halfway houses
		if(lastFireStationCount > 0) {
			b.fireRequest = FIRE_STATION_MAINTENANCE * lastFireStationCount + (halfwayHouseCount * 10);
		}
		else {
			b.fireRequest = 0;
		}
		
		// police request, function of maintenance costs, # of stations, and # of halfway houses
		if(lastPoliceCount > 0) {
			b.policeRequest = POLICE_STATION_MAINTENANCE * lastPoliceCount + (halfwayHouseCount * 10);
		}
		else {
			b.policeRequest = 0;
		}
		...
		// defining public variables values
		roadRequest = b.roadRequest;
		fireRequest = b.fireRequest;
		policeRequest = b.policeRequest;
