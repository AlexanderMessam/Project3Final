...
public class BudgetDialog extends JDialog
{
	...
	{
		// checking if you fully paid police fund requests based on percent
		// if no funding requested, slider doesn't have to be at 100%
		if((engine.policePercent == 1.0 || engine.policeRequest == 0) && 
				(engine.firePercent == 1.0 || engine.fireRequest > 0) && 
				(engine.roadPercent == 1.0 || engine.roadRequest > 0)) {
			engine.cityFullyFunded = true;
		}
		else {
			engine.cityFullyFunded = false;
		}
